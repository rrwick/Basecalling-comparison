...
# This script expects to find the following relative to where it's run:
...
for f in $read_files; do
    ...
    read_data=04_read_data/"$set".tsv
    trimmed_reads=05_trimmed_reads/"$set".fastq.gz
    subsampled_reads=06_subsampled_reads/"$set".fastq.gz
    assembly=07_assemblies/"$set".fasta
    assembly_pieces=08_assembly_data/"$set"_pieces.fasta
    assembly_alignment=08_assembly_data/"$set".paf
    assembly_data=08_assembly_data/"$set".tsv
    nanopolish_assembly_dir=09_nanopolish
    nanopolish_assembly=09_nanopolish/"$set".fasta
    nanopolish_assembly_pieces=10_nanopolish_data/"$set"_pieces.fasta
    nanopolish_assembly_alignment=10_nanopolish_data/"$set".paf
    nanopolish_assembly_data=10_nanopolish_data/"$set".tsv

    printf "\n\n\n\n"
    echo "NORMALISE READ HEADERS: "$set
    echo "--------------------------------------------------------------------------------"
    python3 ../fix_read_names.py $all_reads read_id_to_fast5 | gzip > $all_reads_fixed_names

    printf "\n\n\n\n"
    echo "ASSESS READS: "$set
    echo "--------------------------------------------------------------------------------"
    minimap2 -x map10k -t $threads -c reference.fasta $all_reads_fixed_names > $read_alignment
    python3 ../read_length_identity.py $read_alignment > $read_data
    rm $read_alignment
    ...
    echo "--------------------------------------------------------------------------------"
    python3 ../chop_up_assembly.py $assembly 10000 > $assembly_pieces
    minimap2 -x map10k -t $threads -c reference.fasta $assembly_pieces > $assembly_alignment
    python3 ../read_length_identity.py $assembly_alignment > $assembly_data
    rm $assembly_pieces $assembly_alignment

    printf "\n\n\n\n"
    echo "NANOPOLISH: "$set
    echo "--------------------------------------------------------------------------------"
    python3 ../nanopolish_slurm_wrapper.py $assembly $all_reads_fixed_names $raw_fast5_dir $nanopolish_assembly_dir $nanopolish_exec_dir $threads
    ...
    echo "--------------------------------------------------------------------------------"
    python3 ../chop_up_assembly.py $nanopolish_assembly 10000 > $nanopolish_assembly_pieces
    minimap2 -x map10k -t $threads -c reference.fasta $nanopolish_assembly_pieces > $nanopolish_assembly_alignment
    python3 ../read_length_identity.py $nanopolish_assembly_alignment > $nanopolish_assembly_data
